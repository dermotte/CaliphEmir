...
public class AnnotationFrameProperties {
    private static Logger log = Logger.getLogger(AnnotationFrameProperties.class.getName());
    ...
    private String lastDir = ".", externalViewer = null;
    private String browserWindows = "cmd.exe /c start \"\" \"{url}\"", browserLinux="firefox {url}";
    private String urlHelp = "http://www.semanticmetadata.net/wiki/";
    private String urlHomepage = "http://www.semanticmetadata.net/";
    private String urlDonate = "http://www.semanticmetadata.net/";
    ...
    public AnnotationFrameProperties(File propertyFile, ActionListener listener) {
    ...
    private void initProperties() throws JaxenException {
        ...
        path = new JDOMXPath("properties/browser/windows");
        results = path.selectNodes(propertiesDocument);
        if (results.size() > 0) {
            Element e = (Element) results.get(0);
            browserWindows = e.getTextTrim();
        }

        path = new JDOMXPath("properties/browser/linux");
        results = path.selectNodes(propertiesDocument);
        if (results.size() > 0) {
            Element e = (Element) results.get(0);
            browserLinux = e.getTextTrim();
        }

        path = new JDOMXPath("properties/help/online");
        results = path.selectNodes(propertiesDocument);
        if (results.size() > 0) {
            Element e = (Element) results.get(0);
            urlHelp = e.getTextTrim();
        }

        path = new JDOMXPath("properties/help/homepage");
        results = path.selectNodes(propertiesDocument);
        if (results.size() > 0) {
            Element e = (Element) results.get(0);
            debug("Setting windows browser to " + e.getTextTrim());
            urlHomepage = e.getTextTrim();
        }

        path = new JDOMXPath("properties/help/donate");
        results = path.selectNodes(propertiesDocument);
        if (results.size() > 0) {
            Element e = (Element) results.get(0);
            debug("Setting windows browser to " + e.getTextTrim());
            urlDonate = e.getTextTrim();
        }
    ...
    private JMenu createSubMenu(Element node) {
        ...
        for (Iterator i = subs.iterator(); i.hasNext();) {
            ...
            if (e.getName().equals("menuitem")) {
                ...
                String iconImage = e.getAttributeValue("icon");
                if (iconImage != null) {
//					System.out.println("We want to show: " + iconImage);
//					assert(AnnotationFrameProperties.class.getResource(iconImage)!=null);
                    item.setIcon(new ImageIcon(AnnotationFrameProperties.class.getResource(iconImage)));
                }
    ...
    public void saveConfiguration() {
        try {
            ...
            propertiesRoot.getChild("help").getChild("online").setText(urlHelp);
            propertiesRoot.getChild("help").getChild("homepage").setText(urlHomepage);
            propertiesRoot.getChild("help").getChild("donate").setText(urlDonate);

            propertiesRoot.getChild("browser").getChild("windows").setText(browserWindows);
            propertiesRoot.getChild("browser").getChild("linux").setText(browserLinux);
        } catch (Exception e) {
    ...
    public String getBrowserWindows() {
        return browserWindows;
    }

    public void setBrowserWindows(String browserWindows) {
        this.browserWindows = browserWindows;
    }

    public String getBrowserLinux() {
        return browserLinux;
    }

    public void setBrowserLinux(String browserLinux) {
        this.browserLinux = browserLinux;
    }
